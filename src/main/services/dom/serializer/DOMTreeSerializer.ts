...
export class DOMTreeSerializer {
  ...
  async serializeDOMTree(
    ...
    const simplifiedRoot = await this.createSimplifiedNode(rootNode, previousSelectorMap);
    if (!simplifiedRoot) {
  ...
  private async createSimplifiedNode(
    ...
    if (node.actualChildren) {
      for (const child of node.actualChildren) {
        const simplifiedChild = await this.createSimplifiedNode(child, previousSelectorMap);
        if (simplifiedChild) {
    ...
    if (node.shadowRoots) {
      for (const shadowRoot of node.shadowRoots) {
        const simplifiedShadow = await this.createSimplifiedNode(shadowRoot, previousSelectorMap);
        if (simplifiedShadow) {
   ...
   */
  private isNewElement(node: EnhancedDOMTreeNode, previousSelectorMap?: DOMSelectorMap): boolean {
  ...
  private static buildAttributesString(
    ...
    if (node.tag && node.tag.toLowerCase() === 'input' && node.attributes) {
      ...
      if (['text', ''].includes(inputType)) {
        ...
        if (classAttr.includes('datepicker') || classAttr.includes('datetimepicker') ||
            classAttr.includes('daterangepicker') || node.attributes['data-datepicker']) {
          const dateFormat = node.attributes['data-date-format'] || 'mm/dd/yyyy';
    ...
    if (node.tag && ['input', 'textarea', 'select'].includes(node.tag.toLowerCase())) {
      if (node.axNode && node.axNode.properties) {
        for (const prop of node.axNode.properties) {
          if (String(prop.name) === 'valuetext' && prop.value !== null && prop.value !== undefined) {
          ...
          if (String(prop.name) === 'value' && prop.value !== null && prop.value !== undefined) {
            const valueStr = String(prop.value).trim();
    ...
    if (node._compoundChildren && node._compoundChildren.length > 0) {
      const compoundInfo = node._compoundChildren.map((child: any) => {
        const parts: string[] = [];
        if (child.name) parts.push(`name=${child.name}`);
        if (child.role) parts.push(`role=${child.role}`);
        if (child.valuemin !== undefined && child.valuemin !== null) parts.push(`min=${child.valuemin}`);
        if (child.valuemax !== undefined && child.valuemax !== null) parts.push(`max=${child.valuemax}`);
        if (child.valuenow !== undefined && child.valuenow !== null) parts.push(`current=${child.valuenow}`);

        // Add select-specific information
        if (child.options_count !== undefined && child.options_count !== null) parts.push(`count=${child.options_count}`);
    ...
    if (attributesToInclude.type && attributesToInclude.type.toLowerCase() === node.tag?.toLowerCase()) {
    ...
    booleanAttrs.forEach(attr => {
      if (attributesToInclude[attr] && ['false', '0', 'no'].includes(attributesToInclude[attr].toLowerCase())) {
    ...
    if (Object.keys(attributesToInclude).length > 0) {
      ...
      const formattedAttrs = Object.entries(attributesToInclude).map(([key, value]) => {
        const cappedValue = value.length > 100 ? value.substring(0, 100) : value;
        if (!cappedValue) {
          return `${key}=''`;
        }
  ...
  private static getScrollInfoText(node: EnhancedDOMTreeNode): string {
    ...
    const scrollInfo = node.scrollInfo as any;
    if (!scrollInfo) {
      return '';
    }
    ...
    if (typeof scrollInfo.scrollWidth === 'number' && typeof scrollInfo.clientWidth === 'number' &&
        scrollInfo.scrollWidth > scrollInfo.clientWidth) {
      const horizontalPercent = Math.round((scrollInfo.scrollLeft / (scrollInfo.scrollWidth - scrollInfo.clientWidth)) * 100);
      parts.push(`horizontal: ${horizontalPercent}%`);
    }

    if (typeof scrollInfo.scrollHeight === 'number' && typeof scrollInfo.clientHeight === 'number' &&
        scrollInfo.scrollHeight > scrollInfo.clientHeight) {
      const verticalPercent = Math.round((scrollInfo.scrollTop / (scrollInfo.scrollHeight - scrollInfo.clientHeight)) * 100);
  ...
  static async serializeTree(
    ...
    if (node.excludedByParent) {
      ...
      for (const child of node.children) {
        const childText = await this.serializeTree(child, includeAttributes, depth);
        if (childText) {
    ...
    if (node.originalNode.tag) {
      ...
      if (node.originalNode.tag.toLowerCase() === 'svg') {
        ...
        const attributesString = this.buildAttributesString(node.originalNode, includeAttributes, '');
        if (attributesString) {
      ...
      const isScrollable = node.originalNode.isActuallyScrollable || node.originalNode.isScrollable;
      const shouldShowScroll = node.originalNode.shouldShowScrollInfo;

      if (node.interactiveIndex !== null || isScrollable ||
    ...
    // Process children
    for (const child of node.children) {
      const childText = await this.serializeTree(child, includeAttributes, nextDepth);
      if (childText) {
